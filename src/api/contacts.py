...
async def read_contacts(
    skip: int = 0, limit: int = 20, q: str | None = Query(None, max_length=50), db: AsyncSession = Depends(get_db)
):
    contact_service = ContactService(db)
    contacts = await contact_service.get_contacts(skip, limit, q)
    return contacts

@router.get("/birthday-reminder", response_model=List[ContactResponse])
async def birthday_reminder(db: AsyncSession = Depends(get_db)):
    contact_service = ContactService(db)
    contacts = await contact_service.birthday_reminder()
    return contacts
...
async def remove_contact(contact_id: int, db: AsyncSession = Depends(get_db)):
    contact_service = ContactService(db)
    contact = await contact_service.remove_contact_by_id(contact_id)
    if contact is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found"
        )
